...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await prisma.attendance.findFirst({
      where: { employeeId: session.userId, date: { gte: today, lt: tomorrow }, checkIn: { not: null } }
    })
    if (existing) return NextResponse.json({ error: 'Already checked in today' }, { status: 400 })

    const buffer = Buffer.from(await photo.arrayBuffer())
    const blob = await put(`attendance/${session.userId}/checkin-${Date.now()}.jpg`, buffer, { access: 'public' })
